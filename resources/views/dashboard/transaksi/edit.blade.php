...
@section('title') 
  Dashboard Admin | Edit Data Transaksi
@endsection
...
@section('container')
    ...
    <form method="POST" action="{{ route('transaksi.update', $transaksi->id_transaksi) }}" class="min-w-[300px] w-full rounded-xl mt-5 bg-gray-50 p-4 shadow-sm space-y-4">
        @method('PUT')
        @csrf
        ...
        <div class="space-y-1">
            ...
            <select name="buku_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                @foreach ($buku as $buku) 
                    @if(old('buku_id', $transaksi->buku->buku_id) == $buku->id_buku)
        ...
        <div class="space-y-1">
            ...
            <select name="user_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
                @foreach ($user as $user) 
                    @if(old('user_id', $transaksi->user_id) == $user->id_user)
        ...
        <div class="space-y-1">
            ...
            <input type="date" value="{{ old('tanggal_pinjam', $transaksi->tanggal_pinjam) }}" name="tanggal_pinjam" placeholder="Masukkan Tanggal Pinjam Buku..." class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"/>
        </div>
        ...
        <div class="space-y-1">
            ...
            <input type="date" value="{{ old('tanggal_kembali', $transaksi->tanggal_kembali) }}" name="tanggal_kembali" placeholder="Masukkan Tanggal Kembali Buku..." class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"/>
        </div>
        ...
        <div class="space-y-1">
            ...
            <input type="number" value="{{ old('total_pinjam', $transaksi->total_pinjam) }}" name="total_pinjam" placeholder="Masukkan Jumlah Buku..." class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"/>
        </div>
        @error('total_pinjam')
            <p class="text-red-500 my-1">{{ $message }}</p>
        @enderror
